...
class ArabicReshaper(object):
    ...
    * Python 2: https://docs.python.org/2/library/configparser.html
    ...
    def reshape(self, text):
        ...
        return ''.join(result).replace("ﯼ","ی").replace("ﮎ","ک").replace("ﺭ","ر").replace("ﺩ","د")\
            .replace("ﻭ","و").replace("ﻩ","ه").replace("ﻝ","ل").replace("ﺍ","ا").replace("ﻥ","ن").replace("ﺵ","ش")\
            .replace('ﻡ',"م").replace('ﺯ',"ز").replace('ﺏ',"ب").replace('ﺁ',"آ").replace('ﺱ',"س").replace('ﺕ',"ت")\
            .replace("ﻯ","ی").replace("ﺡ","ح").replace("…","...")
